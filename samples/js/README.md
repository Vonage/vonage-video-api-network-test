...
Make a copy of the samples/js/src/configSample.js file, saving it to samples/js/src/config.js.
...
to generate a unique test session ID (and a corresponding token) for each client.
...
1. Make sure you have built the @vonage/video-client-network-test module locally. See the README
   ...
   package.json file in the /samples/js directory to load it from "*" (or a version, such as
   "^2.x") instead of "file://..":

   ```"@vonage/video-client-network-test": "*"```

2. Run `nvm use` (in the /samples/js directory), or use Node 18 (if you do not use nvm).
   Then run `npm install` (in the /samples/js directory).

3. Run `npm run build` (in the /sample directory). (Run this any time you edit the source code.)

4. Open the /samples/js/index.html page in a web browser.
...
## Opentok support
In order to build a sample app that uses Opentok:
* Build opentok-network-test-js module locally instead of vonage-network-test-js module (please refer to XXX section in the library README file)
* Replace `@vonage/video-client-network-test` package in **package.json** file with
```js
  "@opentok/client": "^2.31.0",
  "opentok-network-test-js": "file:../../../dist"
```
* In **/src/index.ts** replace `import NetworkTest, {ErrorNames, QualityTestResults} from '@vonage/video-client-network-test';` with `import NetworkTest, { ErrorNames, QualityTestResults } from 'opentok-network-test-js';`
* Replace `applicationId` inside **/src/js/config.js** with `apiKey` (The API key corresponding to the OpenTok project the app uses)
* Obtain session ID an token from the [TokBox account page](https://tokbox.com/account).
